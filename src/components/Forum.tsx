...
export function Forum() {
  ...
  const inputClass =
    "w-full bg-[var(--bg-2)] border border-[var(--border)] px-4 py-3 text-sm text-[var(--fg)] outline-none focus:border-[var(--accent)] transition placeholder:text-[var(--muted-2)]";

  return (
    <section id="forum" className="relative py-24 lg:py-32 z-10">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 card p-6 lg:p-8">
            <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
              <h3 className="font-display text-xl font-bold">Recent discussions</h3>
              <div className="flex items-center gap-2 text-xs text-[var(--muted)]">
                {(["all", "pinned", "hot"] as Filter[]).map((f) => (
                  <button
                    key={f}
                    className={`hover:text-[var(--accent)] transition px-2 py-1 border-b-2 capitalize ${
                      filter === f ? "border-[var(--accent)]" : "border-transparent"
                    }`}
                    onClick={() => setFilter(f)}
                  >
                    {f}
            ...
            <div id="forum-threads">
              {loading ? (
                ...
                <p className="text-sm text-[var(--muted)]">No threads yet. Start one above.</p>
              ) : (
                visible.map((t) => (
                  <div key={t.id ?? t.title} className="thread-row py-4 px-2 flex items-center gap-4 cursor-pointer">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="thread-title text-[var(--fg)] font-medium truncate transition-colors">
                        {t.title}
                      </div>
            ...
            <div className="card p-6">
              ...
              <div className="space-y-2">
                {categories.map((c) => (
                  <div
                    key={c.name}
                    className="flex items-center justify-between py-2 px-3 hover:bg-[var(--bg-2)] cursor-pointer transition"
                  >
